...
def dataframes_2():
    ...
    city_frame = pd.DataFrame(
        cities, columns=["name", "country", "population"]
    )
    print(city_frame)

    city_frame.reindex(
        index=[0, 2, 4, 6, 8, 10, 12, 1, 3, 5, 7, 9, 11],
    ...
    city_frame.rename(
        columns={"name": "Nume", "country": "țară", "population": "populație"},
        inplace=True,
    )
...
def dataframes_3():
    ...
    city_frame = pd.DataFrame(
        cities, columns=["name", "population"], index=cities["country"]
    )
...
def dataframes_4():
    ...
    print(
        type(city_frame.population), type(city_frame["population"])
    ...
    city_frame = pd.DataFrame(
        cities, columns=["country", "population"], index=cities["name"]
    )
    idx = 1
    ...
    growth = {
        "Switzerland": {"2010": 3.0, "2011": 1.8, "2012": 1.1, "2013": 1.9},
    ...
    df = pd.DataFrame(
        (np.random.randn(12, 5) * 1000).round(2), columns=names, index=index
    )
    print(df)
