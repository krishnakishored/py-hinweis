...
def basic_series():
    S = pd.Series([11, 28, 72, 3, 5, 8]) # created a default index starting with 0 going to 5
    # print(S)
    print(S.index) # RangeIndex(start=0, stop=6, step=1)
    print(S.values) #  same as ndarrays of Numpy
    X = np.array([11, 28, 72, 3, 5, 8])
    print(type(S.values), type(X)) # <class 'numpy.ndarray'> <class 'numpy.ndarray'>

    fruits = ['apples', 'oranges', 'cherries', 'pears']
    quantities = [20, 33, 52, 10]
    S = pd.Series(quantities, index=fruits)
    print(S.index) # Index(['apples', 'oranges', 'cherries', 'pears'], dtype='object')
    print(S.values)
...
def series_from_dict():
    ...
    d = {"a":23, "b":45, "c":None, "d":0}
    missing = {"c":597939}
    print(pd.Series(d).fillna(missing).astype(int)) # appropriate filling using dictionaries
