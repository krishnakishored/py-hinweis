...
)  # with [] : print(next()) - TypeError: 'list' object is not an iterator
for x in exp:
    print(x, end=" ")
...
next(
    s
)  # We need to call next at least once in order to get to yield where we can send something into the coroutine
s.send("Polo")
s.send("Bolo")
s.send("Solo")
