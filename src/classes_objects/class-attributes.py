...
class C:
    ...
    def __init__(self):
        type(
            self
        ).counter += 1  # we could have written C.counter instead of type(self).counter, because type(self) will be evaluated to "C" anyway
...
if __name__ == "__main__":
    ...
    print(
        x.__dict__
    )  # {'name': 'Marvin', 'build_year': '1979', 'brand': 'HTS', 'energy': 'high'}
    print(
    ...
    for num, text in enumerate(Robot_Asimov.Three_Laws):
        print(
            str(num + 1) + ":" + text
        )  # we can access a class attribute via instance or via the class name
